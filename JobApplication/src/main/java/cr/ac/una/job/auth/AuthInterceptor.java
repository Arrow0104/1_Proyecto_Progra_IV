...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    private static boolean isPublicPath(String uri) {
        if (startsWith(uri, "/css") || startsWith(uri, "/js")
                || startsWith(uri, "/images") || startsWith(uri, "/webjars")) return true;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // /admin/** y /usuarios/** → solo ADMIN
        if ((startsWith(uri, "/admin") || startsWith(uri, "/usuarios")) && rol != Rol.ADMIN) {
            response.sendRedirect("/inicio?error=No%20autorizado"); return false;
        }

        // /empresas/** y /empresa/** → solo EMPRESA
        if ((startsWith(uri, "/empresas") || startsWith(uri, "/empresa")) && rol != Rol.EMPRESA) {
            response.sendRedirect("/inicio?error=No%20autorizado"); return false;
        }

        // /oferentes/** y /oferente/** → solo OFERENTE
        if ((startsWith(uri, "/oferentes") || startsWith(uri, "/oferente")) && rol != Rol.OFERENTE) {
            response.sendRedirect("/inicio?error=No%20autorizado"); return false;
        }
